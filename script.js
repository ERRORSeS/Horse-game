...
// Compatibility shims for older imported/merged builds.
if (typeof window !== 'undefined') {
  if (typeof window.refreshNpcAds !== 'function') window.refreshNpcAds = () => {};
  if (typeof window.seedShowHistory !== 'function') window.seedShowHistory = () => {};
}

function callOptionalHook(name) {
  try {
    const fn = typeof window !== 'undefined' ? window[name] : undefined;
    if (typeof fn === 'function') fn();
  } catch (error) {
    console.warn(`${name} hook failed`, error);
  }
}
...
function createHorseCard(horse) {
  ...
  const illnessLine = activeIssue ? `Active issue: ${activeIssue.name}` : 'No active illness/injury';
  ...
  node.querySelector('.meta').textContent = `${horse.breed} • Personality: ${horse.personality} • Conformation: ${horse.conformation} • COI: ${horse.coi}% • Soundness: ${horse.soundnessYears} years est. • Worth: ${money(horseWorth(horse))} • ${illnessLine} • ${canCompeteUnderSaddle(horse) ? 'Under saddle eligible' : 'In-hand/registry only until age 3'}`;
...
try {
  callOptionalHook('seedShowHistory');
  seed();
  loadGame(false);
  callOptionalHook('refreshNpcAds');
  ensurePanels();
  buildTabs();
  render();
} catch (error) {
